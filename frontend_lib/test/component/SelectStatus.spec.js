...
describe('<SelectStatus />', () => {
  ...
  describe('Static design', () => {
    ...
    it(`should have ${props.availableStatus.length} submenu_item`, () => {
      expect(wrapper.find('.dropdownMenuItem')).to.have.lengthOf(props.availableStatus.length)
    })

    it(`first submenu_item should have its icon: "${props.availableStatus[0].faIcon}"`, () => {
      expect(wrapper.find('.dropdownMenuItem').at(0).children('i').prop('className'))
        .to.equal(`fa-fw ${props.availableStatus[0].faIcon}`)
    })

    it(`second submenu_item should have its icon: "${props.availableStatus[1].faIcon}"`, () => {
      expect(wrapper.find('.dropdownMenuItem').at(1).children('i').prop('className'))
