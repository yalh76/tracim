...
class WebdavTracimContext(TracimContext):
    ...
    def _get_current_workspace_label(self) -> str:
        return convert_file_name_to_bdd(self.path.split("/")[1])
    ...
    def _get_content(self, content_path_fetcher):
        ...
        workspace_name = convert_file_name_to_bdd(splited_local_path[0])
        wapi = WorkspaceApi(
        ...
        if len(splited_local_path) > 2:
            ...
            parents = [convert_file_name_to_bdd(x) for x in parent_string]
        ...
        return content_api.get_one_by_filename_and_parent_labels(
            content_label=convert_file_name_to_bdd(basename(path)),
    ...
    def _get_candidate_workspace_path(self):
        return convert_file_name_to_bdd(self._destpath.split("/")[1])
