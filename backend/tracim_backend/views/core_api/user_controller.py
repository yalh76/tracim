...
from tracim_backend.views.core_api.schemas import UserMessagesMarkAsReadSchema
...
class UserController(Controller):
    ...
    @check_right(has_personal_access)
    @hapic.input_path(UserIdPathSchema())
    @hapic.input_query(UserMessagesMarkAsReadSchema())
    @hapic.output_body(NoContentSchema(), default_http_code=HTTPStatus.NO_CONTENT)
    def set_user_messages_as_read(
        self, context, request: TracimRequest, hapic_data: HapicData
    ) -> None:
        """Mark matching messages as read for user.

        All user messages are marked as read when no filter is given.
        """
        app_config = request.registry.settings["CFG"]  # type: CFG
        event_api = EventApi(request.current_user, request.dbsession, app_config)
        event_api.mark_user_messages_as_read(
            request.candidate_user.user_id, hapic_data.query["content_ids"]
        )
